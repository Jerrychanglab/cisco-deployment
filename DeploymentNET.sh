            ...
            while true; do
                ...
                if [ "${ChooseVxLanStat,,}" = "add" ]; then
                    ...
                    while true; do
                        echo -n -e "\033[1;31m ❯ 填寫: \033[0m"
                        read SwitchIp
                        if [[ $SwitchIp =~ ^10\.252\.252\.[0-9]+$ ]]; then
                            break
                        else
                    ...
                    while true; do
                        ...
                        if [[ $vLanID =~ ^([1-9][0-9]{0,2}|[1-3][0-9]{3}|40[0-8][0-9]|409[0-4])$ ]]; then
                            vLanIdCheck=$(sshpass -p "$password" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null procni@$SwitchIp "show vlan brief | include $vLanID | awk '{print $1}'" 2>/dev/null)
                            if [[ -n $vLanIdCheck ]]; then
                                echo -e "\033[31m ( vLan ID ${vLanID} [已]存在，請重新輸入 ) \033[0m "
                            else
                                break
                            fi
                        else
                    ...
                    #vxlan檢查
                    LoopBackIp=$(sshpass -p "$password" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null procni@$SwitchIp "show running-config interface loopback 0 | begin 'ip address' | head -n 1 | sed 's/ip address //; s/\/.*//' | sed 's/^[[:space:]]*//'" 2>/dev/null)
                    clear
                    echo "========================================================="
                    echo -e $'\e[1;97;44m【 新增 - 參數檢查 】\e[0m '
                    echo -e "\e[32m✔\e[0m Switch IP ➜ \e[1;31m $SwitchIp\e[0m"
                    ...
                    echo -e "\e[32m✔\e[0m LoopBack IP ➜ \e[1;31m $LoopBackIp\e[0m"
                    echo "========================================================="
                    echo ""
                    ...
                    while true; do
                        ...
                        if [[ $vLanID =~ ^([1-9][0-9]{0,2}|[1-3][0-9]{3}|40[0-8][0-9]|409[0-4])$ ]]; then
                            ...
                            if [[ -z $vLanIdCheck ]]; then
                                echo -e "\033[31m ( vLan ID ${vLanID} [不]存在，請重新輸入 ) \033[0m "
                            else
                                break
                            fi
                        else
...
if [ "$confirm" == "yes" ]; then
    ...
    if [ "${ChooseFunction,,}" = "vxlan" ]; then
        ...
        if [ "${ChooseVxLanStat,,}" = "add" ]; then
            echo -e "\n"
            echo -n -e $'\e[1;97;42m【 '${vmName}' 開始部署 】\e[0m '
            echo -e "\n"
            sshpass -p "$password" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null procni@$SwitchIp <<EOF
configure terminal
vlan ${vLanID}
name ${NameRule}
...
exit
EOF
            echo -n -e $'\e[1;97;42m【 '${vmName}' 完成部署 】\e[0m '
            echo -e "\n"
        elif [ "${ChooseVxLanStat,,}" = "del" ]; then
            echo -e "\n"
            echo -n -e $'\e[1;97;42m【 '${vmName}' 開始部署 】\e[0m '
